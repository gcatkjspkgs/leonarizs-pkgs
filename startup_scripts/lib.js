onEvent("loaded", e => {
    ...
    globlal.leonariz.randNum = (min, max, int) => {
        let resul
        if (int) {
    ...
    global.leonariz.circlePos = (event, target, r, particle) => {
        for (let angle = 0; angle < 6.3; angle += 0.1) {
            ...
            event.server.runCommandSilent(`/particle ${particle} ${target.x + x} ${target.y + 1} ${target.z + z} 0.2 0.2 0.2 0 5`)
        }
    }
    ...
    global.leonariz.rayTraceEntities = (event, entity) => {
        ...
        let ray = global.leonariz.rayTrace(event, 200)

        let d = global.leonariz.distance(start.x, (start.y + event.player.getEyeHeight()), start.z, ray.x, ray.y, ray.z)
